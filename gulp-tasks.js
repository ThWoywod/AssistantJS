function gulpTasks(options = {}) {
  ...
  const SOURCES = options.SOURCES || ["src/**/*.ts"];
  ...
  gulp.task("default", ["build-sources", "build-dts", "test"]);

  gulp.task("build", ["build-sources", "build-dts"]);

  gulp.task("build-local", ["build-sources", "build-dts", "local-test-coverage"]);
  ...
  gulp.task(
    "local-test-coverage",
    ["build-sources"],
    shell.task(`nyc --include "${options.COVERAGE_SOURCE || "src"}" --all -e .ts -x "*.spec.ts" jasmine-ts "**/*.spec.ts"`)
  );
  ...
  gulp.task("build-sources-and-maps", ["clean"], function() {
    return gulp
      ...
      .pipe(gulp.dest(options.DESTINATION || "lib/src"));
  });
  ...
  gulp.task("build-sources", ["clean"], function() {
    return gulp
      ...
      .js.pipe(gulp.dest(options.DESTINATION || "lib/src"));
  });
